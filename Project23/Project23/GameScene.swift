...
//  Project23
//
//  Created by David Bochenski on 07/09/2019.
...
class GameScene: SKScene {
    ...
    func tossEnemies() {
        ...
        case .chain:
            ...
            { [weak self] in self?.createEnemy() }
            DispatchQueue.main.asyncAfter(deadline: .now() + (chainDelay / 5.0 * 5))
            { [weak self] in self?.createEnemy() }
        ...
        case .fastChain:
            ...
            { [weak self] in self?.createEnemy() }
            DispatchQueue.main.asyncAfter(deadline: .now() + (chainDelay / 10.0 * 5))
            { [weak self] in self?.createEnemy() }
        }
